...
class RczTests : public QObject {
    Q_OBJECT
private slots:
    void vboExporterGateGeometry_data()
    {
        QTest::addColumn<QString>("producer"); QTest::addColumn<int>("mode");
        QTest::newRow("verified-racechrono") << "Generated by RaceChrono Pro v10.2.4" << 1;
        QTest::newRow("generic-endpoints") << "VBOX endpoint fixture" << 0;
        QTest::newRow("unknown-racechrono") << "Generated by RaceChrono Pro v99.0" << 2;
    }
    void vboExporterGateGeometry()
    {
        ...
        if (mode == 2) {
            ...
            QVERIFY(session.warnings.join(' ').contains("exporter version"));
            return;
        }
        ...
        if (mode == 0) {
            ...
            const auto a = projectCoordinate(gate.endpointA, {50,-20});
    ...
    void optionalPrivatePair()
    {
        ...
        QCOMPARE(referenceLaps.timedLaps.size(), expected.size());
        QVERIFY(!expected.isEmpty());
        double maximumLapError = 0;
        double maximumVboLapError = 0;
        for (qsizetype i = 0; i < expected.size(); ++i) {
            ...
            QVERIFY(error < .1);
            QVERIFY(std::abs(referenceLaps.timedLaps[i].startTelemetryTime + shift - expected[i].first) < .1);
            const double vboError = std::abs(referenceLaps.timedLaps[i].durationSeconds - (expected[i].second - expected[i].first));
            maximumVboLapError = std::max(maximumVboLapError, vboError);
            QVERIFY(vboError < .1);
        }
        qInfo() << "Maximum lap duration difference vs recorded RCZ metadata" << maximumLapError
                << "seconds; VBO maximum lap error" << maximumVboLapError
                << "seconds; VBO-derived laps" << referenceLaps.timedLaps.size();
    }
};
