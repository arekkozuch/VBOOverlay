...
class TelemetryImportTests : public QObject {
    Q_OBJECT
private slots:
    void matchesDatedExportsWithDifferentCoordinateConventions()
    {
        QTemporaryDir directory;
        const auto origin = QDateTime::fromMSecsSinceEpoch(RczFixture::origin + 100, QTimeZone::UTC);
        const auto recording = [&](bool wrongSign) {
            QByteArray text = "File created on " + origin.toString("dd/MM/yyyy 'at' HH:mm:ss").toUtf8()
                + "\n[comments]\nGenerated by RaceChrono Pro v10.2.4\n[column names]\ntime latitude longitude velocity\n[data]\n";
            for (int i = 0; i < 32; ++i) {
                text += origin.addSecs(i).toString("HHmmss.zzz").toUtf8() + ' '
                    + QByteArray::number((50.0 + i * .0001) * 60.0, 'f', 9)
                    + (wrongSign ? " 1200 72\n" : " -1200 72\n");
            }
            return text;
        };
        const auto vboPath = writeSource(directory, "export.vbo", recording(false));
        const auto rczPath = writeSource(directory, "archive.rcz", movingRcz());
        const auto plan = prepareTelemetryImport({rczPath, vboPath});
        QCOMPARE(plan.runs.size(), 2);
        QCOMPARE(plan.possibleSameRuns.size(), 1);
        QCOMPARE(automaticVboPrimaries(plan).value(plan.runs[0].id), plan.runs[1].id);
        const auto wrong = writeSource(directory, "wrong.vbo", recording(true));
        QVERIFY(prepareTelemetryImport({wrong, rczPath}).possibleSameRuns.isEmpty());
    }
