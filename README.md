...
G-Force widgets can invert lateral and longitudinal presentation axes independently, without changing imported telemetry. Alongside the classic target, the widget catalog includes an **F1 G-Force Radar** with a 1.5 g range and 0.25 g concentric-ring steps by default, plus a compact **G-Force Bar** that shows true combined magnitude while clamping only its fill. `Retro Custom` is a generic retro-styled numeric value widget with the usual channel, adjustment, formatting, unit, fallback, and palette settings. Text-bearing value widgets offer **Font size**: `Auto` retains their responsive legacy sizing, while a positive canonical size is scene-scaled consistently for preview and every export resolution.
