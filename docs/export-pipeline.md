...
The offscreen alpha contract is explicit and premultiplied end to end. QRhi readback is wrapped as `QImage::Format_RGBA8888_Premultiplied`; if `QRhi::isYUpInFramebuffer()` reports bottom-to-top rows, the image is vertically flipped exactly once at this boundary. Stage A preserves those premultiplied channel values while changing raw RGBA transport to FFV1/BGRA storage. Stage B marks decoded FFV1 frames `alpha_mode=premultiplied` and declares `overlay=...:alpha=premultiplied`, so FFmpeg does not reinterpret translucent widget edges as straight alpha.
...
Stage A intentionally remains 8-bit RGBA/BGRA because it carries the authored overlay, not decoded source precision. For 10-bit SDR input, Stage B selects a 10-bit overlay/filter output and HEVC Main10 profile; final probing must prove 10-bit pixel format/profile and known color fields before commit. HLG, PQ, and Log/extended sources fail preflight because copying their tags would not make SDR-authored overlay compositing colorimetrically correct.
