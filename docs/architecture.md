...
The modern motorsport broadcast HUD uses one dark translucent panel family (`#101820`, near-white primary text, muted secondary text, restrained borders, and canonical 12 px radii) across Speed, pedals, Retro Custom values, Heart Rate, and the G-Force bar. Throttle is green, active brake is red, and G-Force is amber; a zero brake is represented solely by the neutral track. The analog Retro Tachometer remains circular and integrates its numeric plate, while the F1 radar remains an unboxed circular field with six rings. `VisualSmokeScene.qml` is the deterministic composition review scene: it mounts all of these widgets over a bright roadway backdrop with sample-only data, and the `motorsport-broadcast-smoke` template carries the same nine-widget layout for the editor and export scene.
