...
Qt's private GUI headers are required by CMake and supplied by the matching SDK. Qt Multimedia, Shader Tools, and SVG modules are installed explicitly. The Qt version is pinned because QRhi/GuiPrivate APIs are version-sensitive; changing it still requires local render/export smoke validation. This baseline does not establish compatibility with every later Qt release.
