...
- Injectable multi-volume storage preflight policy, malformed/owned/live manifest recovery rules, and a macOS/Unix helper child/grandchild process-tree shutdown integration test. Windows process-job behavior is compile-tested but not runtime-tested here.
